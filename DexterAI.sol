...
contract DexterAI is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTotal = 2 * 10**11 * 10**18; //200 billion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    

    string private constant _name = "Dexter AI";
    string private constant _symbol = "DXAI";
    uint8 private constant _decimals = 18;

    uint256 private _reflectionFee = 5;
    uint256 private _liquidityTax = 2;
    uint256 private _burningFee = 3;
    uint256 private _donateTax = 5;
    ...
    function _transfer(
        ...
        uint256 currentDonateFee = _donateTax;
    ...
    function _transferStandard(
        ...
        _donateFee(rFees.donateFee, tFees.donateFee);
    ...
    function _donateFee(uint256 rDonateFee, uint256 tDonateFee) private {
        _rOwned[address(this)] = _rOwned[address(this)].add(rDonateFee);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tDonateFee);
    }
        ...
        returns (uint256, Fees memory)
    {
        ...
        tFees.donateFee = tAmount.mul(_donateTax).div(100);
        ...
        )
    {
        ...
        rFees.donateFee = tFees.donateFee.mul(currentRate);
    ...
    function donateFee() public view returns (uint256) {
        return _donateTax;
    }
    ...
    function swapAndLiquify() external nonReentrant onlyOwner returns (bool) {
        ...
        uint256 donateAmount = half.mul(tf).div(denominator);
    ...
    function _sendDonation(uint256 donateAmount) internal {
        _transfer(address(this), donateWallet, donateAmount);
        emit Transfer(address(this), donateWallet, donateAmount);
    }

    

    function setDonateWallet(address _newAddress) external onlyOwner {
        donateWallet = _newAddress;
    }

    

    //Use this function to withdraw stuck ETH
